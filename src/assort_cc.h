  ...
  !###### and allocation to litter (including coarse roots) ############
    if(energyCut==1. .AND. pharv(3, species)>0. .AND. siteType<5.) then ! stump removal: allocate to energywood and/or litter
      ...
      if(stumpsampled==1) then !if stumps are collected
        energyWood(year,ij,7,1) = (stem_assort(2) + felled_croot*0.3/par_rhow)*stumprecoveryrate !note: volume
        energyWood(year,ij,6,1) = energyWood(year,ij,6,1) + energyWood(year,ij,7,1) ! add stumps to total energywood
...
!   energyWood(year,ij,6,1) = stem_assort(5)*energyRatio ! energywood total // ADD STUMP, BRANCHES TO THIS !!
!   energyWood(year,ij,5,1) = stem_assort(6)*energyRatio ! energywood from roundwood (used to meet harvest demand)
   energyWood(year,ij,8,1) = stem_assort(2) ! stump (100% abg, included in energywood if collected)
   energyWood(year,ij,13,1) = stem_assort(7)*n_harvested ! total stemwood according to Laasasenaho taper function
!   energyWood(year,ij,7,1) = 99. !energywood from stumps (if applicable; from stemwood (,,8) + coarse roots, *stumprecoveryrate
...
energyWood(year,ij,10,1) = d_harvested
energyWood(year,ij,11,1) = h_harvested
energyWood(year,ij,12,1) = 88.             ! quality reduction factor (share of potential sawnwood unfit for sawnwood processing)
